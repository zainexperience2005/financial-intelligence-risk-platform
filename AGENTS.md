...
Database schemas are managed through Alembic.

Application startup must not call create_all() to evolve
production schemas.

Any SQLAlchemy schema change requires a corresponding
reviewed migration.

Autogenerated migrations must be inspected before use.

Migrations must be tested against a clean PostgreSQL
database.

Model-generated SQL uses the read-only role and must never
receive migration or DDL privileges.

Do not automatically seed, reset, or destructively modify
production data during application startup or deployment.
